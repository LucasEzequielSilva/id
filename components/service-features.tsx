    ...
    >
      <div className={`space-y-8 ${index % 2 === 0 ? "md:order-1" : "md:order-2"}`}>
        <div className="space-y-4">
          ...
          <h3 className="text-2xl md:text-[2rem] tracking-tight text-[#1A1A18] font-medium">{title}</h3>
          <p className="text-[#6B6B66] text-base leading-relaxed">{description}</p>
        </div>

        <div className="bg-white rounded-xl p-6 border border-[#E5E4DF]">
          <ul className="space-y-4">
            {features.map((feature, i) => (
              ...
              >
                <div className="mt-1 bg-sky-500 p-1 rounded-full flex-shrink-0">
                  <Check className="w-3 h-3 text-white" />
                </div>
                <span className="text-[#3A3A38] text-sm">{feature}</span>
              </motion.li>
            ))}
          </ul>
        </div>
      ...
      <motion.div
        ...
        transition={{ duration: 0.8 }}
      >
        <div className="relative h-full rounded-2xl overflow-hidden border border-[#E5E4DF]">
          <div
